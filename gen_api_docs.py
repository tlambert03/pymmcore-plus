...
AUTOGEN_MARKER = "<!-- generated by gen_api_docs.py -->\n"
...
def _is_autogenerated(path: Path) -> bool:
    """Return True if a file was auto-generated by this script."""
    try:
        text = path.read_text()
    except OSError:
        return False
    if text.startswith(AUTOGEN_MARKER):
        return True
    # Legacy: file containing only a mkdocstrings directive block
    stripped = text.strip()
    return stripped.startswith("::: ") and all(
        line == "" or line.startswith((":::", " ")) for line in stripped.splitlines()
    )


def _clean_autogenerated(directory: Path) -> None:
    """Remove auto-generated .md files and clean up empty directories."""
    if not directory.exists():
        return
    for child in list(directory.iterdir()):
        if child.is_dir():
            _clean_autogenerated(child)
            if not any(child.iterdir()):
                child.rmdir()
        elif child.suffix == ".md" and _is_autogenerated(child):
            child.unlink()
...
def generate_docs(
    ...
    if not dest.exists() or _is_autogenerated(dest):
        dest.write_text(_make_stub(namespace, index_options))
    ...
    for item in sorted(package_root.iterdir()):
        ...
        if item.is_file() and item.suffix == ".py" and not name.startswith("_"):
            ...
            if f"{namespace}.{mod_name}" not in skip:
                dest = docs_dir / f"{mod_name}.md"
                if not dest.exists() or _is_autogenerated(dest):
                    dest.write_text(_make_stub(f"{namespace}.{mod_name}", options))
...
def main(argv: list[str] | None = None) -> None:
    ...
    # Clean auto-generated files (preserves manually authored docs)
    if api_dir.exists():
    ...
    # Create dedicated pages (only if not already manually maintained)
    for fqn, path in dedicated_paths.items():
        path.parent.mkdir(parents=True, exist_ok=True)
        if not path.exists():
            path.write_text(_make_stub(fqn, {}))
            print(f"  dedicated (created): {fqn}")
        elif _is_autogenerated(path):
            print(f"  dedicated (exists, autogenerated): {fqn}")
        else:
